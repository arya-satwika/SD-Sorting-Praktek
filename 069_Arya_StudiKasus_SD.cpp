...
int main()
{
    ...
    double newRating = (tempRating + (stod(titleArr[indexFilm-1][2])*stod(titleArr[indexFilm-1][1])))/(stoi(titleArr[indexFilm-1][2]) + 1);
    int newNRating = stoi(titleArr[indexFilm-1][2]) + 1;
    newRating = trunc(newRating * 10.0)/10.0;
    titleArr[indexFilm-1][1] = to_string(newRating).substr(0, 3);
    titleArr[indexFilm-1][2] = to_string(newNRating);
    selectSort(titleArr);
